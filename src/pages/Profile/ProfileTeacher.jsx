import React, { useEffect, useState } from 'react';
import './ProfileTeacher.css';
import Comment from './components/Comment';
import axios from 'axios';
import CourseList from './components/CourseList';
import { useNavigate } from 'react-router-dom';

export default function ProfileTeacher() {
    const navigate = useNavigate();
    const [user, setUser] = useState({});
    const [teacher, setTeacher] = useState({});
    // const [courses, setCourses] = useState([]);

    useEffect(() => {
        setUser(JSON.parse(sessionStorage.getItem("user")));
        axios.get('http://localhost:4000/api/teachers/info', { params: { UserId: user.UserId } }).then(res => {
            console.log('res', res)
            setTeacher(res.data.teacher);
            // setCourses(res.data.courses);
        })
        // eslint-disable-next-line
    }, [])

    return (
        <div className='main-block w-full h-full grid grid-cols-4 mt-[35px] gap-[20px]'>
            <div className='info-block w-full h-full flex items-center flex-col gap-[20px]'>
                {/* Информация об учителе */}
                <div className="relative w-full">
                    <div className="w-full bg-white rounded-[20px] p-5"> {/* padding замість фіксованої висоти */}
                        <div className="w-full space-y-4"> {/* space-y для відступів між елементами */}
                            <div className="w-full">
                                <div className="w-full flex flex-col items-center">
                                    <img
                                        className="w-[92px] h-[92px] rounded-full"
                                        src="https://placehold.co/92x92"
                                        alt='teacher'
                                    />
                                    <div className="text-black text-[15px] font-normal font-['Mulish']">
                                        {user.FirstName} {user.LastName}
                                    </div>
                                </div>
                                <div className="text-center text-[#827ead] text-xs font-normal font-['Mulish']">
                                    Київ. Україна
                                </div>
                                <div className="text-center text-black text-[15px] font-normal font-['Mulish']">
                                    {user.Email}
                                </div>
                                <div className="text-center text-black text-[15px] font-normal font-['Mulish']">
                                    +380-(93)-365-5453
                                </div>
                            </div>

                            <div className="w-[200px] mx-auto">
                                <div className="w-full h-8 px-4 py-2 rounded-[40px] border border-[#8a48e6] flex justify-center hover:bg-[#632DAE] hover:text-white items-center gap-2.5 text-[#8a48e6] text-[15px] font-bold font-['Nunito']" onClick={() => navigate("/teacher/profile/edit")}>
                                        Редагувати профіль
                                </div>
                            </div>

                            <div className="w-full">
                                <div className="w-full">
                                    <div className="w-full flex justify-between my-[2px]">
                                        <div className="text-black text-[15px] font-normal font-['Mulish']">Предмет:</div>
                                        <div className="text-right text-black text-[15px] font-normal font-['Mulish']">Математика та фізика</div>
                                    </div>
                                    <div className="w-full flex justify-between my-[2px]">
                                        <div className="text-black text-[15px] font-normal font-['Mulish']">Ціна за урок:</div>
                                        <div className="text-right text-black text-[15px] font-normal font-['Mulish']">від {teacher.LessonPrice}грн</div>
                                    </div>
                                </div>
                                <div className="w-full my-1 text-[#827ead] text-xs font-normal font-['Mulish']">
                                    {teacher.AboutTeacher}
                                </div>
                            </div>

                            <div className="w-full flex flex-col items-center">
                                <div data-svg-wrapper className="">
                                    <svg width="130" height="30" viewBox="0 0 130 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M27.5 15C27.5 8.1 21.9 2.5 15 2.5C8.1 2.5 2.5 8.1 2.5 15C2.5 21.05 6.8 26.0875 12.5 27.25V18.75H10V15H12.5V11.875C12.5 9.4625 14.4625 7.5 16.875 7.5H20V11.25H17.5C16.8125 11.25 16.25 11.8125 16.25 12.5V15H20V18.75H16.25V27.4375C22.5625 26.8125 27.5 21.4875 27.5 15Z" fill="#8A48E6" />
                                        <path d="M59.75 2.5H70.25C74.25 2.5 77.5 5.75 77.5 9.75V20.25C77.5 22.1728 76.7362 24.0169 75.3765 25.3765C74.0169 26.7362 72.1728 27.5 70.25 27.5H59.75C55.75 27.5 52.5 24.25 52.5 20.25V9.75C52.5 7.82718 53.2638 5.98311 54.6235 4.62348C55.9831 3.26384 57.8272 2.5 59.75 2.5ZM59.5 5C58.3065 5 57.1619 5.47411 56.318 6.31802C55.4741 7.16193 55 8.30653 55 9.5V20.5C55 22.9875 57.0125 25 59.5 25H70.5C71.6935 25 72.8381 24.5259 73.682 23.682C74.5259 22.8381 75 21.6935 75 20.5V9.5C75 7.0125 72.9875 5 70.5 5H59.5ZM71.5625 6.875C71.9769 6.875 72.3743 7.03962 72.6674 7.33265C72.9604 7.62567 73.125 8.0231 73.125 8.4375C73.125 8.8519 72.9604 9.24933 72.6674 9.54235C72.3743 9.83538 71.9769 10 71.5625 10C71.1481 10 70.7507 9.83538 70.4576 9.54235C70.1646 9.24933 70 8.8519 70 8.4375C70 8.0231 70.1646 7.62567 70.4576 7.33265C70.7507 7.03962 71.1481 6.875 71.5625 6.875ZM65 8.75C66.6576 8.75 68.2473 9.40848 69.4194 10.5806C70.5915 11.7527 71.25 13.3424 71.25 15C71.25 16.6576 70.5915 18.2473 69.4194 19.4194C68.2473 20.5915 66.6576 21.25 65 21.25C63.3424 21.25 61.7527 20.5915 60.5806 19.4194C59.4085 18.2473 58.75 16.6576 58.75 15C58.75 13.3424 59.4085 11.7527 60.5806 10.5806C61.7527 9.40848 63.3424 8.75 65 8.75ZM65 11.25C64.0054 11.25 63.0516 11.6451 62.3483 12.3483C61.6451 13.0516 61.25 14.0054 61.25 15C61.25 15.9946 61.6451 16.9484 62.3483 17.6517C63.0516 18.3549 64.0054 18.75 65 18.75C65.9946 18.75 66.9484 18.3549 67.6517 17.6517C68.3549 16.9484 68.75 15.9946 68.75 15C68.75 14.0054 68.3549 13.0516 67.6517 12.3483C66.9484 11.6451 65.9946 11.25 65 11.25Z" fill="#8A48E6" />
                                        <path d="M115 2.5C108.1 2.5 102.5 8.1 102.5 15C102.5 21.9 108.1 27.5 115 27.5C121.9 27.5 127.5 21.9 127.5 15C127.5 8.1 121.9 2.5 115 2.5ZM120.8 11C120.612 12.975 119.8 17.775 119.388 19.9875C119.212 20.925 118.862 21.2375 118.538 21.275C117.812 21.3375 117.262 20.8 116.562 20.3375C115.462 19.6125 114.838 19.1625 113.775 18.4625C112.538 17.65 113.337 17.2 114.05 16.475C114.238 16.2875 117.438 13.375 117.5 13.1125C117.509 13.0727 117.508 13.0315 117.497 12.9923C117.486 12.953 117.465 12.9171 117.438 12.8875C117.363 12.825 117.262 12.85 117.175 12.8625C117.062 12.8875 115.313 14.05 111.9 16.35C111.4 16.6875 110.95 16.8625 110.55 16.85C110.1 16.8375 109.25 16.6 108.612 16.3875C107.825 16.1375 107.213 16 107.262 15.5625C107.287 15.3375 107.6 15.1125 108.188 14.875C111.838 13.2875 114.262 12.2375 115.475 11.7375C118.95 10.2875 119.663 10.0375 120.138 10.0375C120.238 10.0375 120.475 10.0625 120.625 10.1875C120.75 10.2875 120.787 10.425 120.8 10.525C120.787 10.6 120.812 10.825 120.8 11Z" fill="#8A48E6" />
                                    </svg>
                                </div>
                                <div className="text-[#827ead] text-[10px] font-normal font-['Mulish']">
                                    Зареєстрований користувач: 12.10.2023
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                {/* Подписка */}
                <div className="relative w-full overflow-hidden">

                    <div className="absolute bottom-10" data-svg-wrapper>
                        <svg width="75" height="76" viewBox="0 0 75 76" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M57.136 10.6891C43.8304 -3.50227 12.168 -0.564812 -2 2.67784L-0.768 28.4283C17.712 22.1338 52.824 17.9832 52.824 38.1563C52.824 61.0456 58.552 68.3653 62 72.5C64.9935 76.0897 75 77.0681 75 73.6347C75 70.2013 67.608 69.6291 64.528 61.0456C61.448 52.4621 73.768 28.4283 57.136 10.6891Z" fill="#8A48E6" />
                            <path fill-rule="evenodd" clip-rule="evenodd" d="M74.2473 75.0297C74.3608 74.6861 74.4265 74.3649 74.4265 74.093C74.4265 73.4211 73.3229 73.3511 71.7898 73.254C69.9004 73.1342 67.3587 72.9731 65.4265 71.593C61.9265 69.093 57.9998 56.1122 57.9998 49.593C57.9998 42 57.63 33.9999 53.4265 25.5931C49.634 18.0081 32.9621 14.1016 10 25.5566C28.7577 21.5189 52.7505 21.5737 52.7505 38.2495C52.7505 61.1388 58.4785 68.4585 61.9265 72.5932C64.4481 75.617 71.9461 76.7879 74.2473 75.0297Z" fill="#AB85E5" />
                        </svg>
                    </div>
                    <div className="w-full h-72 bg-[#120C38] rounded-[20px] p-5"> {/* padding замість фіксованої висоти */}
                        <div className="podpiska-bg w-full h-full absolute inset-0 rounded-[20px] opacity-10" />

                        <div className="space-y-4 relative z-10"> {/* space-y для відступів, z-10 для видимості поверх фону */}
                            <div className="h-[25px] text-white text-[32px] font-bold font-['Nunito']">
                                Навчай на повну!
                            </div>
                            <div className="text-[#827ead] text-[15px] font-normal font-['Mulish']">
                                Оформлюй підписку для розширення можливостей
                            </div>

                            <div className="absolute top-[76px] left-1/2" data-svg-wrapper>
                                <svg width="177" height="212" viewBox="0 0 177 212" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <g clip-path="url(#clip0_75_627)">
                                        <path d="M0.182785 246.15C0.284969 246.984 0.450781 247.805 0.678245 248.604C2.04175 253.804 5.40295 257.848 9.1407 260.131C10.3263 260.862 11.554 261.497 12.815 262.03C23.1206 266.629 35.7211 265.931 45.5471 259.627C47.8205 258.161 49.8643 256.256 51.5917 253.993C55.0401 242.827 53.9263 230.422 53.411 218.789C52.8918 206.948 52.0079 195.108 51.5322 183.244C51.0368 171.065 47.9927 121.174 47.4774 109C45.1533 109.674 42.7957 110.181 40.4181 110.516C40.5846 117.037 43.2759 161.295 43.3988 167.825C43.6049 177.864 44.1241 187.851 44.35 197.867C44.5958 207.757 45.2577 218.249 42.614 227.779C40.0772 236.828 34.5479 242.48 28.1465 243.567C25.9145 243.961 23.6363 243.803 21.4598 243.104C19.0459 242.333 15.9107 240.863 14.7731 237.558C14.722 237.451 14.6808 237.338 14.6502 237.221C13.909 234.526 14.8365 231.421 16.097 229.013C18.4276 224.539 21.3568 222.662 24.7616 220.231C27.1992 218.475 28.7252 212.989 24.496 213.229C20.9643 213.419 17.0046 213.539 13.6395 215.415C7.79701 218.651 3.17536 227.256 1.13407 235.104C0.218458 238.64 -0.29682 242.49 0.182785 246.15Z" fill="#5224AA" />
                                        <path d="M137.808 249.667C137.706 250.522 137.54 251.364 137.313 252.184C135.953 257.517 132.589 261.664 128.851 264.006C127.667 264.756 126.441 265.407 125.182 265.954C114.878 270.671 102.275 269.955 92.4509 263.489C90.1793 261.983 88.1362 260.029 86.4073 257.711C82.9594 246.258 84.073 233.536 84.5922 221.604C85.1074 209.46 85.9951 197.315 86.4707 185.147C86.9621 172.657 90.0057 121.486 90.5249 109C92.8472 109.692 95.2031 110.211 97.5791 110.555C97.4166 117.243 94.7217 162.636 94.5989 169.334C94.3928 179.63 93.8776 189.873 93.6478 200.145C93.402 210.29 92.7402 221.05 95.3836 230.824C97.9199 240.106 103.452 245.903 109.849 247.017C112.08 247.421 114.358 247.26 116.534 246.543C118.948 245.751 122.106 244.244 123.22 240.855C123.274 240.747 123.316 240.63 123.343 240.509C124.088 237.745 123.157 234.56 121.9 232.09C119.566 227.501 116.637 225.577 113.233 223.083C110.796 221.282 109.27 215.655 113.499 215.902C117.03 216.096 120.993 216.22 124.353 218.144C130.195 221.462 134.816 230.289 136.861 238.338C137.788 241.964 138.303 245.912 137.808 249.667Z" fill="#5224AA" />
                                        <path d="M128.004 1.84827C102.648 -4.0136 66.301 4.17889 53.8893 24.923C23.2843 27.8892 0 49.9674 0 71.5943C0 95.2968 27.9767 116.37 62.8557 116.37C63.2504 116.37 63.645 116.335 64.0397 116.331C86.0532 116.331 93.0898 123.511 113.031 125.434C137.764 127.82 160.07 126.894 173.188 90.0666C186.764 51.9174 162.382 9.80926 128.004 1.84827Z" fill="#8A48E6" />
                                        <path d="M65.3257 261.526C63.9593 264.998 61.9687 268.163 59.4619 270.85C59.1612 271.222 58.8362 271.571 58.4892 271.896C57.3313 273.044 56.0826 274.087 54.7566 275.015C54.1596 275.429 53.5309 275.843 52.9062 276.231C51.7652 276.886 50.5918 277.477 49.391 278.002C48.9956 278.167 48.6714 278.329 48.2918 278.461C47.352 278.853 46.3892 279.182 45.4094 279.445C40.892 280.768 36.1881 281.252 31.511 280.877C31.3529 280.877 31.2303 280.844 31.0721 280.844C28.2035 280.624 25.3588 280.141 22.571 279.4C14.663 277.365 7.38761 273.133 3.21613 265.422C-7.79578 245.153 11.0886 207.923 35.3069 215.406C35.5618 215.478 35.8126 215.565 36.0581 215.667C36.0898 215.667 36.1214 215.7 36.153 215.7C36.7759 215.936 37.3237 216.348 37.7346 216.892C37.7742 216.949 37.8414 216.987 37.877 217.045C37.9126 217.103 37.877 217.045 37.877 217.069C37.9442 217.181 38.0549 217.247 38.1063 217.376C38.8971 219.146 37.4143 221.049 35.9435 222.265C31.4873 225.938 25.9042 228.271 22.2349 232.83C17.7787 238.311 16.9959 246.894 20.7284 252.917C27.2842 263.482 41.2102 260.992 46.295 250.555C51.2178 240.412 49.652 227.804 50.0593 216.85C50.3716 208.316 50.0593 199.749 49.7746 191.253C49.2408 174.086 48.193 156.956 46.9198 139.826C46.1685 129.584 45.3118 119.345 44.3497 109.108L68.912 101V224.999C68.9041 236.979 69.9084 250.104 65.3257 261.526Z" fill="#8A48E6" />
                                        <path d="M69.0954 101V225.028C69.0954 237 68.083 250.147 72.6544 261.564C78.3647 275.84 92.2846 281.942 106.481 280.883C117.487 280.056 129.022 276.002 134.772 265.44C145.809 245.17 126.926 207.939 102.673 215.41C101.566 215.75 100.379 216.279 99.9049 217.375C99.114 219.142 100.601 221.074 102.064 222.277C106.544 225.963 112.108 228.288 115.798 232.834C120.251 238.32 121.034 246.899 117.289 252.923C110.74 263.48 96.8125 261.01 91.723 250.573C86.7839 240.438 88.3775 227.821 87.9702 216.842C87.6538 208.332 87.9702 199.765 88.2352 191.26C88.7809 174.101 89.817 156.958 91.0824 139.844C91.8456 129.589 92.6642 119.334 93.6765 109.104L69.0954 101Z" fill="#8A48E6" />
                                        <path d="M92.3479 121.055C98.3347 122.828 104.499 124.614 113.024 125.434C137.756 127.819 160.062 126.893 173.18 90.0664C178.875 74.0777 177.884 57.3985 172.3 42.8223C177.067 57.63 157.591 78.9391 143.569 80.74C141.282 81.0084 139.13 81.9608 137.399 83.4713C135.667 84.9818 134.438 86.9791 133.873 89.1993C126.88 117.751 92.3479 121.055 92.3479 121.055Z" fill="#5224AA" />
                                        <path d="M76.7001 41.3862C76.7001 41.2685 75.9542 34.9633 74.0046 31.487C80.6308 30.8592 105.707 39.4166 110.861 45.0548C96.543 41.2293 76.9211 41.3862 76.708 41.3862H76.7001Z" fill="#120C38" />
                                        <path d="M44.2006 38.2748C44.2006 38.2748 43.4981 31.8832 44.6386 28.0263C38.0362 28.866 15.4701 42.7045 11.6934 49.3354C24.8983 42.4259 44.2006 38.2748 44.2006 38.2748Z" fill="#120C38" />
                                        <path d="M86.6097 80.2652C92.2767 80.2652 96.8706 75.6979 96.8706 70.0639C96.8706 64.4298 92.2767 59.8625 86.6097 59.8625C80.9428 59.8625 76.3489 64.4298 76.3489 70.0639C76.3489 75.6979 80.9428 80.2652 86.6097 80.2652Z" fill="white" />
                                        <path d="M88.0857 77.1224C91.2984 77.1224 93.9028 73.7338 93.9028 69.5537C93.9028 65.3737 91.2984 61.9851 88.0857 61.9851C84.873 61.9851 82.2686 65.3737 82.2686 69.5537C82.2686 73.7338 84.873 77.1224 88.0857 77.1224Z" fill="#120C38" />
                                        <path d="M69.0635 118.885C69.0635 118.885 68.4124 158.361 68.4124 180.349C68.4124 202.336 68.7794 234.188 68.7794 234.188C68.7794 234.188 69.3477 195.317 69.3477 180.082C69.3477 164.846 69.0951 118.301 69.0951 118.301" fill="#120C38" />
                                        <path d="M86.6097 46.7419C80.8354 46.7446 75.2746 48.9128 71.038 52.8136C66.8013 56.7143 64.2007 62.0605 63.7556 67.7842H57.5004C57.0413 61.9048 54.3099 56.4306 49.8792 52.5099C45.4485 48.5892 39.6628 46.5267 33.7357 46.7551C27.8086 46.9835 22.2008 49.485 18.0887 53.7349C13.9765 57.9847 11.6794 63.6527 11.6794 69.5498C11.6794 75.4469 13.9765 81.1149 18.0887 85.3648C22.2008 89.6147 27.8086 92.1162 33.7357 92.3446C39.6628 92.573 45.4485 90.5105 49.8792 86.5898C54.3099 82.6691 57.0413 77.1949 57.5004 71.3155H63.7556C64.1032 75.736 65.7399 79.9599 68.4651 83.4699C71.1903 86.9799 74.8858 89.6237 79.0992 91.0776C83.3126 92.5315 87.8609 92.7324 92.1874 91.6558C96.5139 90.5791 100.431 88.2715 103.458 85.0157C106.486 81.7598 108.493 77.6969 109.233 73.3243C109.974 68.9518 109.416 64.4595 107.628 60.3974C105.84 56.3352 102.9 52.8796 99.1664 50.4535C95.4333 48.0274 91.0696 46.7362 86.6097 46.738V46.7419ZM34.6502 88.9834C30.7839 88.9842 27.0041 87.845 23.789 85.7099C20.5739 83.5748 18.0679 80.5398 16.588 76.9886C15.108 73.4374 14.7206 69.5296 15.4747 65.7595C16.2288 61.9894 18.0906 58.5263 20.8245 55.8082C23.5584 53.0901 27.0417 51.2392 30.8338 50.4894C34.6259 49.7397 38.5565 50.1249 42.1284 51.5963C45.7003 53.0676 48.7531 55.5591 50.9006 58.7555C53.0482 61.952 54.194 65.7098 54.1932 69.5538C54.1932 72.1053 53.6877 74.6319 52.7056 76.9892C51.7235 79.3465 50.2839 81.4884 48.4692 83.2926C46.6545 85.0968 44.5001 86.528 42.129 87.5045C39.7579 88.4809 37.2166 88.9834 34.6502 88.9834ZM86.6097 88.9834C82.7437 88.9827 78.9647 87.8422 75.7506 85.7061C72.5366 83.5701 70.0318 80.5345 68.5531 76.9831C67.0743 73.4318 66.688 69.5243 67.443 65.7547C68.198 61.9851 70.0604 58.5227 72.7946 55.8054C75.5289 53.0881 79.0122 51.238 82.8041 50.4889C86.596 49.7398 90.5261 50.1254 94.0976 51.597C97.669 53.0686 100.721 55.5601 102.869 58.7564C105.016 61.9526 106.161 65.7102 106.161 69.5538C106.161 72.106 105.655 74.6332 104.672 76.991C103.69 79.3488 102.249 81.4911 100.434 83.2954C98.6182 85.0997 96.4629 86.5307 94.0909 87.5067C91.7189 88.4827 89.1768 88.9845 86.6097 88.9834Z" fill="#120C38" />
                                        <path d="M22.3372 71.9942C22.3843 72.3744 22.5695 72.7243 22.8581 72.978C23.1467 73.2318 23.5187 73.3718 23.9039 73.3718C24.2892 73.3718 24.6611 73.2318 24.9497 72.978C25.2383 72.7243 25.4236 72.3744 25.4707 71.9942C25.4707 71.9432 25.447 71.904 25.443 71.8569H25.4865C25.4865 69.5051 26.4261 67.2497 28.0988 65.5867C29.7714 63.9238 32.04 62.9896 34.4055 62.9896C36.771 62.9896 39.0396 63.9238 40.7123 65.5867C42.3849 67.2497 43.3246 69.5051 43.3246 71.8569H43.368C43.368 71.904 43.3404 71.9432 43.3404 71.9942C43.3875 72.3744 43.5728 72.7243 43.8613 72.978C44.1499 73.2318 44.5219 73.3718 44.9071 73.3718C45.2924 73.3718 45.6643 73.2318 45.9529 72.978C46.2415 72.7243 46.4268 72.3744 46.4739 71.9942C46.4739 71.9432 46.4502 71.904 46.4463 71.8569H46.4739C46.4739 68.6747 45.2024 65.6229 42.9391 63.3728C40.6759 61.1226 37.6062 59.8585 34.4055 59.8585C31.2048 59.8585 28.1352 61.1226 25.8719 63.3728C23.6086 65.6229 22.3372 68.6747 22.3372 71.8569H22.3648C22.3608 71.904 22.3372 71.9432 22.3372 71.9942Z" fill="#120C38" />
                                    </g>
                                    <defs>
                                        <clipPath id="clip0_75_627">
                                            <rect width="177" height="272" fill="white" />
                                        </clipPath>
                                    </defs>
                                </svg>
                            </div>

                            <div className="w-[142px] h-10 px-4 py-2 bg-[#8a48e6] rounded-[40px] flex justify-center items-center gap-2.5 mt-[228px]">
                                <div className="text-white text-[15px] font-bold font-['Nunito']">Детальніше</div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div className='big-block w-full h-full flex flex-col items-start col-span-3 gap-[20px]'>
                {/* Блоки */}
                <div className='blocks w-full grid grid-cols-3 gap-[20px]'>
                    {/* Блок 1 */}
                    <div className='uchni w-full h-[180px] flex items-center'>
                        <div className="w-full h-[180px] bg-white rounded-[20px] flex justify-center items-center flex-col">
                            <div className="text-center text-[#8a48e6] text-[32px] font-bold font-['Nunito']">158</div>
                            <div className="text-center text-[#120c38] text-2xl font-bold font-['Nunito']">Учнів</div>
                        </div>
                    </div>
                    {/* Блок 2 */}
                    <div className='rating w-full h-[180px] flex items-center'>
                        <div className="w-full h-[180px] bg-white rounded-[20px] flex justify-center items-center flex-col">
                            <div className="text-center text-[#120c38] text-2xl font-bold font-['Nunito']">Рейтинг</div>
                            {/* Звёзды */}
                            <div data-svg-wrapper className="stars">
                                <svg width="200" height="40" viewBox="0 0 200 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M19.0489 2.92705C19.3483 2.00574 20.6517 2.00574 20.9511 2.92705L24.2658 13.1287C24.3996 13.5407 24.7836 13.8197 25.2168 13.8197H35.9434C36.9122 13.8197 37.3149 15.0593 36.5312 15.6287L27.8532 21.9336C27.5027 22.1883 27.3561 22.6396 27.4899 23.0517L30.8046 33.2533C31.104 34.1746 30.0495 34.9407 29.2658 34.3713L20.5878 28.0664C20.2373 27.8117 19.7627 27.8117 19.4122 28.0664L10.7342 34.3713C9.95048 34.9407 8.896 34.1746 9.19535 33.2533L12.5101 23.0517C12.6439 22.6396 12.4973 22.1883 12.1468 21.9336L3.46877 15.6287C2.68505 15.0593 3.08783 13.8197 4.05655 13.8197H14.7832C15.2164 13.8197 15.6004 13.5407 15.7342 13.1287L19.0489 2.92705Z" fill="#FFA869" />
                                    <path d="M59.0489 2.92705C59.3483 2.00574 60.6517 2.00574 60.9511 2.92705L64.2658 13.1287C64.3996 13.5407 64.7836 13.8197 65.2168 13.8197H75.9434C76.9122 13.8197 77.3149 15.0593 76.5312 15.6287L67.8532 21.9336C67.5027 22.1883 67.3561 22.6396 67.4899 23.0517L70.8046 33.2533C71.104 34.1746 70.0495 34.9407 69.2658 34.3713L60.5878 28.0664C60.2373 27.8117 59.7627 27.8117 59.4122 28.0664L50.7342 34.3713C49.9505 34.9407 48.896 34.1746 49.1954 33.2533L52.5101 23.0517C52.6439 22.6396 52.4973 22.1883 52.1468 21.9336L43.4688 15.6287C42.6851 15.0593 43.0878 13.8197 44.0566 13.8197H54.7832C55.2164 13.8197 55.6004 13.5407 55.7342 13.1287L59.0489 2.92705Z" fill="#FFA869" />
                                    <path d="M99.0489 2.92705C99.3483 2.00574 100.652 2.00574 100.951 2.92705L104.266 13.1287C104.4 13.5407 104.784 13.8197 105.217 13.8197H115.943C116.912 13.8197 117.315 15.0593 116.531 15.6287L107.853 21.9336C107.503 22.1883 107.356 22.6396 107.49 23.0517L110.805 33.2533C111.104 34.1746 110.05 34.9407 109.266 34.3713L100.588 28.0664C100.237 27.8117 99.7627 27.8117 99.4122 28.0664L90.7342 34.3713C89.9505 34.9407 88.896 34.1746 89.1954 33.2533L92.5101 23.0517C92.6439 22.6396 92.4973 22.1883 92.1468 21.9336L83.4688 15.6287C82.6851 15.0593 83.0878 13.8197 84.0566 13.8197H94.7832C95.2164 13.8197 95.6004 13.5407 95.7342 13.1287L99.0489 2.92705Z" fill="#FFA869" />
                                    <path d="M139.049 2.92705C139.348 2.00574 140.652 2.00574 140.951 2.92705L144.266 13.1287C144.4 13.5407 144.784 13.8197 145.217 13.8197H155.943C156.912 13.8197 157.315 15.0593 156.531 15.6287L147.853 21.9336C147.503 22.1883 147.356 22.6396 147.49 23.0517L150.805 33.2533C151.104 34.1746 150.05 34.9407 149.266 34.3713L140.588 28.0664C140.237 27.8117 139.763 27.8117 139.412 28.0664L130.734 34.3713C129.95 34.9407 128.896 34.1746 129.195 33.2533L132.51 23.0517C132.644 22.6396 132.497 22.1883 132.147 21.9336L123.469 15.6287C122.685 15.0593 123.088 13.8197 124.057 13.8197H134.783C135.216 13.8197 135.6 13.5407 135.734 13.1287L139.049 2.92705Z" fill="#FFA869" />
                                    <path d="M179.049 2.92705C179.348 2.00574 180.652 2.00574 180.951 2.92705L184.266 13.1287C184.4 13.5407 184.784 13.8197 185.217 13.8197H195.943C196.912 13.8197 197.315 15.0593 196.531 15.6287L187.853 21.9336C187.503 22.1883 187.356 22.6396 187.49 23.0517L190.805 33.2533C191.104 34.1746 190.05 34.9407 189.266 34.3713L180.588 28.0664C180.237 27.8117 179.763 27.8117 179.412 28.0664L170.734 34.3713C169.95 34.9407 168.896 34.1746 169.195 33.2533L172.51 23.0517C172.644 22.6396 172.497 22.1883 172.147 21.9336L163.469 15.6287C162.685 15.0593 163.088 13.8197 164.057 13.8197H174.783C175.216 13.8197 175.6 13.5407 175.734 13.1287L179.049 2.92705Z" stroke='grey' />
                                </svg>
                            </div>
                            <div className="text-center text-[#8a48e6] text-2xl font-bold font-['Nunito']">4</div>
                        </div>
                    </div>
                    {/* Блок 3 */}
                    <div className='w-full h-[180px] flex items-center'>
                        <div className="w-full h-[180px] bg-white rounded-[20px] flex justify-center items-center flex-col">
                            <div className="text-center text-[#8a48e6] text-[32px] font-bold font-['Nunito']">50</div>
                            <div className="text-center text-[#120c38] text-2xl font-bold font-['Nunito']">Авторських навчальних матеріалів</div>
                        </div>
                    </div>
                </div>
                {/* Блок с курсами */}
                <CourseList />
                {/* Комментарии */}
                <div className="w-full bg-white rounded-[20px] flex flex-col p-[20px]">
                    <div className="text-center text-[#120c38] text-2xl font-bold font-['Nunito']">Коментарі учнів</div>
                    <div className="comments w-full grid grid-cols-2">
                        {/* ком 1 */}
                        <Comment text="Допомагла підтягнути знання шкілної програми з математики. Уроки цікаві та продуктивні, дуже доступно викладається матеріал.Дуже радий що потрапив до Восьменко О. С. Попри заняття в групі з 4х людей, кожному учню завжди приділяли достатньо уваги і всі все розуміли." />
                        {/* ком 2 */}
                        <Comment text="Добрий і гарний викладач. Допомога підготуватись до вступних іспитів. Заняття структуровані, а також зрозумілі і цікаві. Рекомендую!" />
                    </div>
                    <div className="w-[215px] h-[15px] mt-[20px] text-[#8a48e6] text-[15px] font-bold font-['Nunito']">Читати більше коментарів ></div>
                </div>
            </div>
        </div>
    );
}
