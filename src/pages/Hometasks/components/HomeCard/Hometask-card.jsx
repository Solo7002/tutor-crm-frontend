...
export const StatusButton = ({ status, mark, maxmark, onClick, disabled = false, children, svg, className, ...props }) => {
    ...
    const getSvg = () => {
        ...
        // if (status === "pending") {
        //     return (
        //         <svg width="32" height="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-8 h-8 md:w-8 md:h-8 sm:w-6 sm:h-6">
    ...
    return (
        <button
            ...
            className={`rounded-3xl outline outline-1 justify-end items-center flex transition-all duration-300 ${styleStatus === "default"
            ...
            {...props}
        >
            <div className={`grow shrink basis-0 text-center text-base font-bold font-['Nunito'] md:text-base sm:text-sm`}>{getText()}</div>
            {getSvg() && <div data-svg-wrapper className="flex items-center justify-center">{getSvg()}</div>}
        </button>
    );
};
...
export const HomeTaskCardFull = ({ status = "default", subject, title, issuedDate, dueDate, teacher, photoSrc, mark = 0, maxmark = 0, onClick }) => {
    ...
    return (
        <div className="hometask-card relative pattern-bg" style={{
            backgroundImage: 'linear-gradient(to top right, white, transparent)'
        }}>
            <div className={`w-full max-w-[420px] h-auto md:h-60 px-4 py-3 back-card bg-gradient-to-tr homework-status-${statusColors[status]} from-white to-emerald-30 rounded-3xl border flex flex-col md:flex-row justify-start items-start gap-4 overflow-hidden`}>
                <div className="w-full md:w-48 flex-col justify-start items-start gap-2 inline-flex">
                    ...
                    <div className="self-stretch h-auto md:h-44 flex-col justify-start items-start gap-4 flex">
                        <div className={`self-stretch h-auto md:h-20 text-slate-900 text-xl md:text-xl sm:text-lg font-normal font-['Nunito'] class-title`}>{title}</div>
                        <div className="h-auto md:h-20 flex-col justify-center items-start md:items-end gap-1 flex w-full">
                            <div className="self-stretch justify-start items-center gap-2 inline-flex">
                                <div data-svg-wrapper>
                                    <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 md:w-6 md:h-6 sm:w-5 sm:h-5">
                                ...
                                <div><span className="text-slate-400 text-base md:text-base sm:text-sm font-normal font-['Nunito'] info-text">Видано:<br /></span><span className="text-slate-900 font-bold font-['Nunito'] md:text-base sm:text-sm">{issuedDate}</span></div>
                            </div>
                            {/* <div data-svg-wrapper className="w-full md:w-auto self-start">
                                <svg width="105" height="2" viewBox="0 0 105 2" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full max-w-[200px]">
                            ...
                            <div className="self-stretch justify-start items-center gap-2 inline-flex">
                                ...
                                <div><span className="text-slate-400 text-base md:text-base sm:text-sm font-normal font-['Nunito'] info-text">{status == 'done' ? 'Виконано' : 'Виконати до'}<br /></span><span className={`text-base md:text-base sm:text-sm font-bold homework-status-${statusColors[status]} font-['Nunito']`}>{dueDate}</span></div>
                            </div>
                        </div>
                    </div>
                </div>
                <div className="w-full md:flex-grow md:shrink md:basis-0 flex-col justify-center items-center md:items-end gap-2 inline-flex mt-4 md:mt-0">
                    ...
                    <div className="self-stretch h-auto md:h-44 relative">
                        <img alt="img" className={`w-full md:w-44 h-40 object-cover rounded-tl-3xl rounded-tr-3xl`} src={photoSrc} />
                        <StatusButton 
                            status={status} 
                            mark={mark} 
                            maxmark={maxmark} 
                            className={`w-full md:w-44 h-11 px-4 py-2 left-0 bottom-0 md:top-[135px] gap-5 ${status === 'done' ? 'absolute' : 'mt-1 md:absolute'}`} 
                            onClick={onClick} 
                        />
                    </div>
                </div>
...
export const HomeTaskCard = ({ status = "default", subject, title, issuedDate, dueDate, teacher, mark = 0, maxmark = 0, onClick }) => {
    ...
    return (
        <div className="hometask-card">
            <div className={`w-full max-w-[408px] h-auto md:h-[184px] p-4 back-card bg-gradient-to-tr from-white to-emerald-30 rounded-3xl border flex-col justify-start items-start gap-3 inline-flex homework-status-${statusColors[status]}`}>
                <div className="self-stretch h-auto md:h-20 flex-col justify-start items-start gap-2 flex">
                    <div className="self-stretch flex justify-between items-center gap-2">
                        <div className="h-6 text-slate-400 text-base md:text-base sm:text-sm font-bold font-['Nunito']">
                            {subject}
                        </div>
                        <div className="text-right text-slate-400 text-base md:text-base sm:text-sm font-bold font-['Nunito']">
                            {teacher}
                        </div>
                    </div>
                    <div className="self-stretch text-slate-900 text-xl md:text-xl sm:text-lg font-normal font-['Nunito'] class-title">
                        {title}
                    </div>
                </div>
                <div className="self-stretch justify-between items-end flex-col md:flex-row flex md:inline-flex gap-3 md:gap-0">
                    <div className="w-full md:w-48 flex-col justify-start items-start gap-2 inline-flex">
                        <div className="self-stretch justify-start items-center gap-2 inline-flex">
                            <div data-svg-wrapper>
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 md:w-6 md:h-6 sm:w-5 sm:h-5">
                            ...
                            <div>
                                <span className="text-slate-400 text-base md:text-base sm:text-sm font-['Nunito'] info-text">Видано:</span>
                        ...
                        <div className="self-stretch justify-start items-center gap-2 inline-flex">
                            ...
                            <div>
                                <span className="text-slate-400 text-base md:text-base sm:text-sm font-['Nunito'] info-text">{status == 'done' ? 'Виконано' : 'Виконати до'}: </span>
                    ...
                    <StatusButton 
                        ...
                        maxmark={maxmark} 
                        onClick={onClick} 
                        className={`w-full md:w-36 h-12 p-2 gap-2`} 
                    />
                </div>
            </div>
        </div>
