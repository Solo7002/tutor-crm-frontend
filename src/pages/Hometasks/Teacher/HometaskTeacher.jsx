...
const HometaskTeacher = () => {
  ...
  const [sortOption, setSortOption] = useCookieState('HometaskTeacher_sortOption', t('HomeTaskTeacher.components.page.AtFirstNew'));
  ...
  useEffect(() => {
    ...
    if (token) {
      try {
        ...
        axios.get(`${process.env.REACT_APP_BASE_API_URL}/api/hometasks/hometask-data-by-teacher-id/${teacherId}`,
          {
            headers:{
  ...
  const sortedHometasks = [...filteredHometasks].sort((a, b) => {
    if (sortOption === t('HomeTaskTeacher.components.page.AtFirstNew')) {
      return new Date(b.HometaskStartDate) - new Date(a.HometaskStartDate);
    } else if (sortOption === t('HomeTaskTeacher.components.page.AtFirstOld')) {
      return new Date(a.HometaskStartDate) - new Date(b.HometaskStartDate);
    } else if (sortOption === t('HomeTaskTeacher.components.page.ByAlphabet')) {
  ...
  const renderUserCards = () => {
    ...
    if (students.length === 0) {
      return <div>{t('HomeTaskTeacher.components.page.NoTasksYet')}</div>;
    }
        ...
        <div className="w-full sm:w-[210px] stroke-purple-600 hover:stroke-white mt-3 sm:mt-0">
          <SecondaryButton onClick={() => setIsManagerPage(!isManagerPage)}>
            <div className="relative">
              ...
              <div className="ml-7 text-sm md:text-base">
                {isManagerPage
                  ? t('HomeTaskTeacher.components.page.TaskVerification')
                  : t('HomeTaskTeacher.components.page.TaskManager')}
              </div>
            </div>
          </SecondaryButton>
        </div>
      ...
      {isManagerPage ? (
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mt-4 md:mt-6 gap-3">
            <div className="flex items-center gap-2">
              ...
              <div className="mt-1">
                <SortDropdown
                  options={[
                    t('HomeTaskTeacher.components.page.AtFirstNew'),
                    t('HomeTaskTeacher.components.page.AtFirstOld'),
                    t('HomeTaskTeacher.components.page.ByAlphabet')
                  ]}
                  ...
                  <div className="w-full h-full relative rounded-3xl">
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center">
                      ...
                      <div className="text-[#120c38] text-base md:text-[20px] font-normal font-['Mulish'] mt-3 md:mt-5 transition-all duration-300 ease-in-out group-hover:scale-105 group-hover:font-semibold">
                        {t('HomeTaskTeacher.components.page.AddTask')}
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div>
          <div className="w-full flex justify-between items-center mt-4 md:mt-7 mr-0 md:mr-3">
            <div className="font-[Nunito] font-[700] text-xl md:text-2xl leading-none tracking-tighter align-middle text-[#120C38]">
              {t('HomeTaskTeacher.components.page.RecentTasks')}
            </div>
            <div
            ...
            >
              {t('HomeTaskTeacher.components.page.More')}
            </div>
          </div>
              ...
              <div className="h-[calc(100vh-300px)] overflow-y-auto p-3 md:p-5 bg-white rounded-t-3xl border border-[#d7d7d7] mx-auto mt-4 md:mt-6">
                <div className="flex flex-wrap gap-2 md:gap-4 items-center mb-4">
                  <FilterButton
                    ...
                    type={"check"}
                    text={t('HomeTaskTeacher.components.page.ToCheck')}
                  ...
                  <FilterButton
                    ...
                    type={"done"}
                    text={t('HomeTaskTeacher.components.page.Checked')}
                    number={selectedHometask2 ? selectedHometask2.DoneHometasks.filter(dh => dh.Mark !== -1).length : "0"}
                    className="flex-1 min-w-[120px]"
                  />
                  <FilterButton
                    ...
                    type={"not_done"}
                    text={t('HomeTaskTeacher.components.page.NotSubmitted')}
