...
export default function MaterialList({ name, ext, img, onDownloadClick, materialId = null, onDelete, onEdit, openAccessModalHandler }) {
    ...
    const handleSave = () => {
        if (newName.trim() !== '' && newName !== name) {
            ...
            toast.success(
                <div>
                    <p>{t("MaterialComponents.MaterialList.nameChanged")}</p>
                    <p>{t("MaterialComponents.MaterialList.newName")}: {newName}</p>
                </div>
            );
        } else {
            setIsEditing(false);
    ...
    const handleDelete = () => {
        toast.info(
            <div>
                <p>{t("MaterialComponents.MaterialList.deleteConfirmation")}</p>
                <div className="flex gap-2 mt-2">
                    <button
                        ...
                        onClick={() => {
                            ...
                            toast.success(
                                <div>
                                    <p>{t("MaterialComponents.MaterialList.deleteSuccess")}</p>
                                    <p>{t("MaterialComponents.MaterialList.name")}: {name}</p>
                                </div>,
                                { autoClose: 5000 }
                            );
                        }}
                    >
                        {t("MaterialComponents.MaterialList.delete")}
                    </button>
                    <button
                        className="px-3 py-1 bg-gray-300 text-gray-800 rounded-md hover:bg-gray-400"
                        onClick={() => toast.dismiss()}
                    >
                        {t("MaterialComponents.MaterialList.cancel")}
                    </button>
                </div>
            </div>,
        ...
        <div className="w-full h-[74px] p-[15px] bg-white rounded-3xl border border-[#d7d7d7] transition-shadow duration-200 hover:shadow-[0_0_0_3px_#8A48E6] justify-start items-center gap-5 inline-flex relative">
            ...
            <div className="grow shrink basis-0 flex-col justify-start items-start gap-2.5 inline-flex">
                {isEditing ? (
                    <input
                        ...
                        autoFocus
                        placeholder={t("MaterialComponents.MaterialList.enterNewName")}
            ...
            >
                ...
                >
                    ...
                    <div className="justify-center text-[#e64851] text-[15px] font-bold font-['Nunito']">{t("MaterialComponents.MaterialList.delete")}</div>
                </button>
                ...
                >
                    ...
                    <div className="justify-center text-[#120c38] text-[15px] font-bold font-['Nunito']">{t("MaterialComponents.MaterialList.changeName")}</div>
                </button>
                ...
                >
                    ...
                    <div className="justify-center text-[#120c38] text-[15px] font-bold font-['Nunito']">{t("MaterialComponents.MaterialList.manageAccess")}</div>
                </button>
