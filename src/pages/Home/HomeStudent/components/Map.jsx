...
const Map = ({ locations }) => {
  ...
  const [position, setPosition] = useState([50.4501, 30.5234]); // Default: Kyiv
  ...
  useEffect(() => {
      if (navigator.geolocation) {
          navigator.geolocation.getCurrentPosition(
              ...
              () => {
                  toast.error(t('HomeStudent.Map.error_location'));
              }
          );
      } else {
          toast.error(t('HomeStudent.Map.error_location'));
  ...
  const formats = [t('HomeStudent.Map.format'), 'Online', 'Offline'];
  const lessonTypes = [t('HomeStudent.Map.lesson_type'), 'Group', 'Individual'];
  const subjects = [t('HomeStudent.Map.all_subjects'), ...new Set(locations.map((loc) => loc.subject))];

  useEffect(() => {
      let filtered = locations;
      if (selectedFormat !== t('HomeStudent.Map.format') && selectedFormat !== 'all') {
          filtered = filtered.filter((loc) => loc.format === selectedFormat);
      }
      if (selectedLessonType !== t('HomeStudent.Map.lesson_type') && selectedLessonType !== 'all') {
          filtered = filtered.filter((loc) => loc.lessonType === selectedLessonType);
      }
      if (selectedSubject !== t('HomeStudent.Map.all_subjects') && selectedSubject !== 'all') {
          filtered = filtered.filter((loc) => loc.subject === selectedSubject);
      }
      setFilteredLocations(filtered);
  }, [selectedFormat, selectedLessonType, selectedSubject, locations, t]);
    ...
    <div className="relative w-full h-[33vh] rounded-[20px] shadow-md overflow-hidden ">
      <div className="absolute top-[20px] left-[60px] flex gap-4 z-[9999]">
        <div className="w-[156px] h-12 p-2 bg-white rounded-2xl border border-[#d7d7d7] flex justify-between items-center">
          <div className="text-[#827ead] text-[15px] font-bold font-['Nunito']">{t('HomeStudent.Map.all_subjects')}</div>
        ...
        <div className="w-[156px] h-12 p-2 bg-white rounded-2xl border border-[#d7d7d7] flex justify-between items-center">
          <div className="text-[#827ead] text-[15px] font-bold font-['Nunito']">{t('HomeStudent.Map.format')}</div>
          <div data-svg-wrapper>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M12 14.5V15M12 15L18 9M12 15L6 9" stroke="#827FAE" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
        ...
        <div className="w-[156px] h-12 p-2 bg-white rounded-2xl border border-[#d7d7d7] flex justify-between items-center">
          <div className="text-[#827ead] text-[15px] font-bold font-['Nunito']">{t('HomeStudent.Map.lesson_type')}</div>
          <div data-svg-wrapper>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M12 14.5V15M12 15L18 9M12 15L6 9" stroke="#827FAE" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
      ...
      >
        ...
        {position && (
          <Marker position={position} icon={customIcon}>
            <Popup>
            {t('HomeStudent.Map.current_location')}
            </Popup>
          </Marker>
        )}
        {/* Center user */}
