...
const Productivity = ({ productivityData }) => {
  ...
  const filterDataByPeriod = (data, selectedPeriod) => {
    ...
    switch (selectedPeriod) {
      case "За день":
        filtered.tasksChecked = Math.round(data.tasksChecked * 0.05);
        filtered.lessonsConducted = Math.round(data.lessonsConducted * 0.05);
        filtered.newClients = Math.round(data.newClients * 0.05);
        filtered.reviewsReceived = Math.round(data.reviewsReceived * 0.05);
        break;
      case "За тиждень":
        filtered.tasksChecked = Math.round(data.tasksChecked * 0.2);
        filtered.lessonsConducted = Math.round(data.lessonsConducted * 0.2);
        filtered.newClients = Math.round(data.newClients * 0.2);
        filtered.reviewsReceived = Math.round(data.reviewsReceived * 0.2);
        break;
      case "За місяць":
        filtered.tasksChecked = Math.round(data.tasksChecked * 0.5);
        filtered.lessonsConducted = Math.round(data.lessonsConducted * 0.5);
        filtered.newClients = Math.round(data.newClients * 0.5);
        filtered.reviewsReceived = Math.round(data.reviewsReceived * 0.5);
        break;
      case "За 3 місяці":
        filtered.tasksChecked = Math.round(data.tasksChecked * 0.75);
        filtered.lessonsConducted = Math.round(data.lessonsConducted * 0.75);
        filtered.newClients = Math.round(data.newClients * 0.75);
        filtered.reviewsReceived = Math.round(data.reviewsReceived * 0.75);
        break;
      case "За пів року":
        filtered.tasksChecked = Math.round(data.tasksChecked * 0.9);
        filtered.lessonsConducted = Math.round(data.lessonsConducted * 0.9);
        filtered.newClients = Math.round(data.newClients * 0.9);
        filtered.reviewsReceived = Math.round(data.reviewsReceived * 0.9);
        break;
      case "За рік":
      case "За весь час":
      default:
  ...
  const fullStars = Math.floor(filteredData.rating); // Целая часть рейтинга
  const fractionalPart = filteredData.rating - fullStars; // Дробная часть рейтинга
