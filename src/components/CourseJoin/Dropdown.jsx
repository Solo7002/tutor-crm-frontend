...
export default function Dropdown({ textAll = "Усі предмети", options, onSelectSubject }) {
  ...
  return (
    <div className="relative w-[420px] border border-grey-1 rounded-2xl">
      {/* Заголовок dropdown */}
      <button
      ...
      {/* Выпадающий список */}
      {isOpen && (
        <ul className="absolute w-full mt-1 bg-white border border-[#d7d7d7] rounded-2xl shadow-lg z-10 max-h-60 overflow-y-auto">
          {[textAll, ...options.map(subject => subject.SubjectName)].map((option) => (
            option === selected ? null : (
              <li
                ...
                className="p-2 hover:bg-[#f4f4f5] hover:rounded-2xl cursor-pointer text-[#827ead] text-[15px] font-bold font-['Nunito']"
              >
                {option}
              </li>
            )
